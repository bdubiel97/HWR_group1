...
from styleClassification.OtherAttempts.CNN.model import RecognitionModel
...
def validate_model(model_name, class_names):
    ...
    for folder in listdir(input_folder):
        ...
        s = ', '.join(["{}: {} votes".format(p, n) for p, n in image_count])
        s += "\nInput image '{}' has been classified as '{}'".format(folder, c)
        print(s)
        with open('results.txt', 'a') as fr:
            fr.write(s + '\n')


if __name__ == '__main__':
    input_folder = path.join('..', '..', '..', 'Segmentation', 'output')
    model_folder = path.join('trained_models', "latest")
